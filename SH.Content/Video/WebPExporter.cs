...
public static class WebpExporter
{
    ...
    /// <param name="ct"></param>
    /// <returns></returns>
    public static async Task<bool> TryExportAsync(Clip clip, string outputDir, double scale, double playbackSpeed, ILogger log, CancellationToken ct)
    {
        try
        {
            string path = IOUtils.CombineAsOSPath(outputDir, $"{clip.Name}.webp");
            log?.Debug($@"Exporting clip to ""{path}""");
